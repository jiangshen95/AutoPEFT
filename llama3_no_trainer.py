...
def get_dataset(name):
    ...
    if name in [
            'axb', 'axg', 'cb', 'copa', 'multirc', 'record', 'rte', 'wic', 'wsc'
    ]:
...
def preprocessing(task_name, configs=None):
    ...
    peft_config = ConfigUnion(
        *[config_list[i] for i in range(len(config_list))])
    ...
    valid_dataloader = DataLoader(dataset['test'], batch_size=16, shuffle=False)
...
def train_epoch(epoch_num):
    ...
    for epoch in range(epoch_num):
        ...
        for batch in tqdm(
                ...
                unit="batch"):
            text_fields = [
                field for field in dataset['test'].column_names
        ...
        # 验证
        model.eval()
        total_loss, total_correct, total_count = 0, 0, len(dataset['test'])
        ...
        layer_number, layer_type = pruning_methods.prune_model(model, p_method = "gradient", gradients = gradients)
        print(layer_number)
        print(layer_type)
